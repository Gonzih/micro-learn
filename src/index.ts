...
import {
  ...
  createProfile,
  getConcept,
  getConceptsByProfile,
  getDueConcepts,
  getConceptByLesson,
  createConcept,
  updateConcept,
  getOrCreateSession,
  incrementSessionConcepts,
  getStreak,
  getUpcomingReviews,
} from "./db.js";
import { applyRating, todayString, addDays, encouragementMessage } from "./spaced-repetition.js";
import type { Rating } from "./spaced-repetition.js";
    ...
    tools: [
      {
        name: "subscribe",
        description:
          "Subscribe to a learning domain. Creates a learner profile and returns your first lesson.",
        inputSchema: {
          type: "object",
          properties: {
            profileId: {
              type: "string",
              description:
                "Unique identifier for the learner (e.g. your name or email). Defaults to MICRO_LEARN_PROFILE_ID env var.",
            },
            domain: {
              type: "string",
              description:
                "Domain to learn (e.g. 'system-design', 'financial-literacy'). Defaults to MICRO_LEARN_DOMAIN env var.",
            },
            dailyTimeMinutes: {
              type: "number",
      ...
      },
      {
        ...
        description:
          "Get today's micro-lesson. Returns a due spaced-repetition review first, or a new lesson if nothing is due.",
        inputSchema: {
          type: "object",
          properties: {
            profileId: {
      ...
      },
      {
        name: "rate_lesson",
        description:
          "Rate your understanding of a lesson after studying it. Updates spaced repetition schedule.",
        inputSchema: {
          type: "object",
          properties: {
            profileId: {
            ...
            rating: {
              ...
              description:
                "'know' = understood well, 'fuzzy' = partially understood, 'forgot' = didn't remember.",
            },
          },
...
async function handleSubscribe(a: Record<string, unknown>) {
  ...
  if (!domain) {
    return errorContent(
      `Unknown domain '${domainId}'. Use get_domains to see available options.`
    );
  }
...
async function handleGetDailyLesson(a: Record<string, unknown>) {
  const profileId = str(a.profileId) || process.env.MICRO_LEARN_PROFILE_ID;
  if (!profileId) return errorContent("profileId is required.");

  const profile = getProfile(profileId);
  if (!profile) {
    return errorContent(
      `Profile '${profileId}' not found. Use subscribe first.`
    );
  }
  ...
  if (dueConcepts.length > 0) {
    ...
    try {
      lesson = JSON.parse(due.lesson_content) as Lesson;
    } catch {
    ...
    const seenNames = new Set(
      getConceptsByProfile(profileId).map((c) => c.concept_name)
    );
    ...
    if (unseen.length === 0) {
      return jsonContent({
        message:
          "You've covered all available lessons in this domain! Keep reviewing due concepts or use subscribe to add another domain.",
        lesson: null,
        conceptId: null,
      });
    }
  ...
  return jsonContent({
    ...
    instructions:
      "Read through the lesson, then use rate_lesson with 'know', 'fuzzy', or 'forgot' based on how well you understood and remembered the material.",
  });
}

async function handleRateLesson(a: Record<string, unknown>) {
  const profileId = str(a.profileId) || process.env.MICRO_LEARN_PROFILE_ID;
  if (!profileId) return errorContent("profileId is required.");

  const conceptId = str(a.conceptId);
  if (!conceptId) return errorContent("conceptId is required.");

  const rating = str(a.rating) as Rating;
  if (!["know", "fuzzy", "forgot"].includes(rating)) {
    return errorContent("rating must be 'know', 'fuzzy', or 'forgot'.");
  }

  const concept = getConcept(conceptId);
  if (!concept) return errorContent(`Concept '${conceptId}' not found.`);
  if (concept.profile_id !== profileId) {
    return errorContent("Concept does not belong to this profile.");
  }

  const result = applyRating(
    concept.interval_days,
    concept.ease_factor,
...
async function handleGetProgress(a: Record<string, unknown>) {
  const profileId = str(a.profileId) || process.env.MICRO_LEARN_PROFILE_ID;
  if (!profileId) return errorContent("profileId is required.");

  const profile = getProfile(profileId);
  if (!profile) return errorContent(`Profile '${profileId}' not found.`);
  ...
  const retentionRate =
    totalReviews > 0 ? Math.round((knowReviews / concepts.length) * 100) : 0;
...
function handleSearchConcept(a: Record<string, unknown>) {
...
async function handleGetUpcomingReviews(a: Record<string, unknown>) {
  const profileId = str(a.profileId) || process.env.MICRO_LEARN_PROFILE_ID;
  if (!profileId) return errorContent("profileId is required.");
