...
export async function sendDailyReminder(profile) {
    if (!profile.telegram_chat_id)
        return;
    const db = getDb();
    const domain = profile.domain;
    const message = `🧠 *Time for your daily micro-lesson!*\n\n` +
...
export async function sendWeeklySummary(profile) {
    if (!profile.telegram_chat_id)
        return;
    const db = getDb();
    ...
    const sessions = db
        .prepare(`SELECT * FROM daily_sessions WHERE profile_id = ? AND date >= ? AND date <= ?`)
        .all(profile.id, fromDate, today);
