export const LESSONS = [
    // ─── SYSTEM DESIGN (10) ───────────────────────────────────────────────────
    {
        ...
        appliedExample: "HBase and Zookeeper are CP: they'll refuse reads/writes if they lose quorum, ensuring you never see inconsistent data. DynamoDB and Cassandra are AP by default: they keep serving requests during partitions, but you might get eventual consistency. Cassandra lets you tune consistency level per query — QUORUM for strong consistency, ONE for maximum availability.",
        questionRecall: "What are the three properties in the CAP Theorem, and why can't you have all three in a distributed system?",
        questionApplication: "You're building a shopping cart service. Briefly losing the latest item added is acceptable, but the cart must always be accessible. Which CAP trade-off do you choose, and why?",
        keyTakeaway: "Network partitions are inevitable; your real choice is CP (consistency wins) vs AP (availability wins). Design your system knowing which you need before you need it.",
        estimatedMinutes: 5,
    },
    {
        id: "sd-002",
        ...
        appliedExample: "AWS Application Load Balancer (ALB) is Layer 7 — it can route /api/* to your API servers and /static/* to your CDN origin servers in the same rule set. AWS Network Load Balancer (NLB) is Layer 4 — millions of requests per second with ultra-low latency. Netflix uses Ribbon, a client-side load balancer, where each service instance discovers peers and balances calls itself without a central bottleneck.",
        questionRecall: "What is the difference between Round Robin and Least Connections load balancing algorithms?",
        questionApplication: "Your video transcoding jobs take wildly different amounts of time (10 seconds to 10 minutes). Which load balancing algorithm would you choose for distributing jobs to worker servers, and why?",
        keyTakeaway: "Load balancers are the foundation of horizontal scaling. Choose your algorithm based on request homogeneity: round robin for uniform work, least connections for variable-duration tasks.",
        estimatedMinutes: 5,
    },
    {
        id: "sd-003",
        ...
        appliedExample: "Twitter caches your home timeline in Redis. When you log in, your timeline (a list of tweet IDs) is read from Redis — not recomputed from billions of follows. Cache invalidation — knowing when to expire stale data — is the famously hard problem. Phil Karlton said: 'There are only two hard things in Computer Science: cache invalidation and naming things.'",
        questionRecall: "Describe the Cache-Aside pattern. What happens on a cache hit vs a cache miss?",
        questionApplication: "Your e-commerce product page queries 5 different database tables. It's called 10,000 times per minute. Product data changes about once per hour. Design a caching strategy including what to cache and what TTL to set.",
        keyTakeaway: "Cache-Aside is the default pattern. Set TTLs aggressively for read-heavy, slow-changing data. Cache invalidation is hard — prefer TTL over manual invalidation when possible.",
        estimatedMinutes: 5,
    },
    {
        id: "sd-004",
        ...
        appliedExample: "Instagram shards by user ID using a library called ids that encodes the shard number in every generated ID. All data for a given user (photos, follows, likes) lives on the same shard, so user-centric queries are fast. For feed generation across users, they use a separate fanout service. Vitess (used by YouTube, Slack) sits in front of MySQL and handles sharding transparently.",
        questionRecall: "What is the difference between horizontal scaling (sharding) and vertical scaling? What is a hot spot in range-based sharding?",
        questionApplication: "You're sharding a multi-tenant SaaS database by tenant_id. One tenant (a Fortune 500 company) generates 80% of all traffic. What problem does this cause and how would you address it?",
        keyTakeaway: "Shard by a key that distributes load evenly and co-locates related data. Hash-based sharding distributes evenly; range-based is simpler but risks hot spots. Avoid cross-shard transactions.",
        estimatedMinutes: 5,
    },
    {
        id: "sd-005",
        ...
        appliedExample: "When you book an Uber, an OrderCreated event fires. The driver-matching service, pricing service, notification service, and analytics service all subscribe to it independently. Adding a new fraud-detection service requires zero changes to the booking service. Kafka processes 1.7 trillion events per day across LinkedIn, handling everything from user activity to site metrics.",
        questionRecall: "What is the difference between an event-driven (pub/sub) architecture and a traditional request/response (REST) architecture?",
        questionApplication: "You're building an e-commerce system. A user places an order. List three downstream services that need to react and explain why event-driven architecture is better than the order service calling each one directly.",
        keyTakeaway: "Events decouple services: the publisher doesn't need to know who cares. This makes systems resilient and extensible — new consumers can subscribe without touching existing code.",
        estimatedMinutes: 5,
    },
    {
        id: "sd-006",
        ...
        appliedExample: "Netflix built its own CDN (Open Connect) and placed servers inside ISPs worldwide. During peak hours, 95% of Netflix traffic is served directly from ISPs' networks. Cloudflare's CDN has 300+ PoPs and serves 20% of all web traffic. For a typical SPA, putting your JavaScript bundle on a CDN cuts load time for international users by 60–80%.",
        questionRecall: "What is a CDN edge node and how does it reduce latency for end users?",
        questionApplication: "Your web app is hosted in US-East. You have users in Brazil, India, and Germany who complain about slow load times. You have static assets (images, JS) and a dynamic API. What would you put on a CDN and what would you keep at origin?",
        keyTakeaway: "CDNs slash latency by serving from nodes close to users. Aggressively cache static assets (long TTL). Dynamic content requires careful cache-key design or should remain at origin.",
        estimatedMinutes: 5,
    },
    {
        id: "sd-007",
        ...
        appliedExample: "Netflix's Hystrix (now in maintenance mode, succeeded by Resilience4j) pioneered circuit breakers in microservices. When Netflix's recommendation service is slow, Hystrix trips the breaker and serves cached or generic recommendations instead — degraded but functional. The 2012 AWS US-East outage cascaded because services kept retrying failed calls without circuit breakers, amplifying load on an already struggling system.",
        questionRecall: "Name the three states of a Circuit Breaker and describe what happens in each state.",
        questionApplication: "Your payment service calls a third-party fraud-detection API that occasionally becomes slow (10s timeouts). How would you implement a circuit breaker, and what fallback behavior would you use when the breaker is open?",
        keyTakeaway: "Fail fast, not slow. A circuit breaker turns a slow failure (thread exhaustion) into a fast failure (immediate rejection), preventing one struggling service from dragging down the entire system.",
        estimatedMinutes: 5,
    },
    {
        id: "sd-008",
        ...
        appliedExample: "GitHub's REST API allows 5,000 requests/hour for authenticated users, 60 for unauthenticated. Stripe rate-limits by API key with burst allowances. Redis is the standard backend for distributed rate limiters — INCR + EXPIRE for fixed window, sorted sets for sliding window. Cloudflare processes 6 trillion requests daily and uses rate limiting as a first line of DDoS defense.",
        questionRecall: "What is the Token Bucket algorithm for rate limiting? What advantage does it have over a fixed window counter?",
        questionApplication: "You're building a public API for your SaaS product. Free users get 100 requests/hour, Pro users get 10,000/hour. Requests come from multiple servers (distributed). How do you implement this, and where do you store the counters?",
        keyTakeaway: "Token Bucket is the most versatile algorithm: it allows controlled bursting while enforcing average rate limits. Use Redis for distributed rate limit state, keyed by user/API key.",
        estimatedMinutes: 5,
    },
    {
        id: "sd-009",
        ...
        appliedExample: "Amazon's DynamoDB, Apache Cassandra, and Riak all use consistent hashing with virtual nodes. Cassandra's ring topology means adding a node to a 10-node cluster only moves ~10% of data. Akamai's CDN uses consistent hashing to map URLs to edge servers. Memcached client libraries implement consistent hashing so that cache clusters can be scaled without cache stampedes.",
        questionRecall: "Why does standard modular hashing cause massive cache misses when a server is added or removed? How does consistent hashing solve this?",
        questionApplication: "You have a 5-node Memcached cluster with 1TB of cached data. You need to add 2 more nodes. With consistent hashing and 150 vnodes per server, approximately what percentage of cached data needs to move?",
        keyTakeaway: "Consistent hashing limits remapping to ~1/N of keys when the cluster changes. Virtual nodes are essential for even load distribution. It's the foundation of most distributed cache and database ring topologies.",
        estimatedMinutes: 5,
    },
    {
        id: "sd-010",
        ...
        appliedExample: "A bank account in CQRS: the write side enforces 'balance cannot go negative', emits TransactionCompleted events. The read side maintains a pre-built view: daily_balance_summary table, perfect for the dashboard query with no JOINs. Greg Young popularized CQRS paired with Event Sourcing. Microsoft's Azure Cosmos DB change feed, Kafka Streams, and AWS DynamoDB Streams are all used to update CQRS read models in production systems.",
        questionRecall: "What two concerns does CQRS separate, and what is the main trade-off you accept when adopting it?",
        questionApplication: "Your social media app has 10M users. The 'user profile page' query joins 6 tables and takes 800ms. You don't want to add more indexes to the write tables. How would CQRS help, and what would the read model look like?",
        keyTakeaway: "CQRS lets reads and writes optimize independently. The read model is a denormalized projection built for query speed, updated asynchronously from write-side events. Accept eventual consistency as the price of that flexibility.",
        estimatedMinutes: 5,
    },
    // ─── FINANCIAL LITERACY (10) ──────────────────────────────────────────────
    {
        ...
        appliedExample: "If you invest $500/month from age 25 to 65 at 7% average annual return, you accumulate ~$1.3 million. If you wait until 35, same contributions, same rate, you get ~$609,000 — half as much, despite only 10 fewer years. Warren Buffett made 99% of his wealth after age 50 — he calls himself 'a snowball rolling down a very long hill.'",
        questionRecall: "What is the Rule of 72? How would you use it to estimate how long it takes $20,000 to grow to $40,000 at 8% annual return?",
        questionApplication: "You have $5,000 in credit card debt at 20% APR and $5,000 you could invest in an index fund averaging 8% annually. What does compound interest tell you about which to prioritize first?",
        keyTakeaway: "Time is the dominant variable in compound growth. Start early, even with small amounts. The same math that builds wealth in investments destroys wealth in high-interest debt — eliminate high-rate debt first.",
        estimatedMinutes: 5,
    },
    {
        id: "fl-002",
        ...
        appliedExample: "Vanguard's S&P 500 Index Fund (VFIAX) has a 0.04% expense ratio. A comparable actively managed large-cap fund charges 0.75%. On $100,000 invested for 30 years at 7% gross return: index fund grows to $741,000; active fund to $640,000 — $100,000 less just from fees. Jack Bogle founded Vanguard and created the first retail index fund in 1976. The industry called it 'Bogle's Folly.' Today index funds hold over $15 trillion.",
        questionRecall: "What is an expense ratio, and why does even a 1% difference in expense ratio matter so much over time?",
        questionApplication: "Your 401(k) offers a target-date fund with 0.12% expense ratio, an S&P 500 index fund at 0.04%, and an actively managed growth fund at 1.2%. You have 30 years until retirement. How do you allocate, and what's your reasoning?",
        keyTakeaway: "In investing, costs compound just like returns — but in reverse. Low-cost index funds outperform most active managers over the long run. Minimize fees, maximize time in market.",
        estimatedMinutes: 5,
    },
    {
        id: "fl-003",
        ...
        appliedExample: "If you invest $7,000/year in a taxable account earning 7%, you pay ~15% capital gains tax annually, netting ~5.95% effective return. In a Roth IRA, you earn the full 7%. Over 30 years: Roth IRA ≈ $708,000, taxable account ≈ $567,000 — $141,000 difference purely from tax shelter. Microsoft offers a 50% 401(k) match up to 6% of salary — an employee earning $100,000 who contributes 6% gets $3,000 free annually from the employer.",
        questionRecall: "What is the key difference between a Traditional IRA and a Roth IRA in terms of when you pay taxes?",
        questionApplication: "You're 28 years old, in the 22% marginal tax bracket, and expect to be in the 32% bracket at retirement. You can afford $500/month for investing. What accounts do you prioritize and why?",
        keyTakeaway: "Max tax-advantaged accounts before taxable investing. Roth accounts favor younger investors and those expecting higher future tax rates. Always capture the full employer match — it's free money.",
        estimatedMinutes: 5,
    },
    {
        id: "fl-004",
        ...
        appliedExample: "Marcus by Goldman Sachs, Ally, and SoFi HYSAs currently offer 4.5–5% APY — 10x the national average savings rate of 0.46%. On a $15,000 emergency fund, that's $675/year in interest, essentially free money for keeping your cash accessible. During COVID-19, workers without emergency funds exhausted retirement accounts early — paying 10% penalty + income tax, permanently destroying compound growth.",
        questionRecall: "Why is a High-Yield Savings Account better for an emergency fund than investing in a stock index fund?",
        questionApplication: "Your monthly essential expenses are $3,500. You currently have $2,000 in savings and $8,000 in student loans at 5% interest and $3,000 in credit card debt at 22%. In what order do you build your emergency fund vs pay down debt?",
        keyTakeaway: "Build a starter emergency fund ($1,000) before attacking debt, then complete 3–6 months after high-interest debt is gone. Keep it in a HYSA — accessible but earning meaningful interest.",
        estimatedMinutes: 5,
    },
    {
        id: "fl-005",
        ...
        appliedExample: "During the 2020 COVID crash, the S&P 500 fell 34% in 33 days. Investors who sold locked in losses. DCA investors who kept buying automatically purchased shares at 34% off. By August 2020, the index had fully recovered. Contributing $500/month to your 401(k) regardless of news is DCA in action. Vanguard found that lump sum investing beats DCA ~67% of the time over 12 months — but only if you actually invest rather than waiting for a dip that may never come.",
        questionRecall: "How does Dollar-Cost Averaging result in a lower average cost per share over time compared to investing a fixed share count?",
        questionApplication: "You receive a $50,000 inheritance. You know you should invest it, but you're terrified of investing it all right before a crash. How would you think about lump sum vs DCA here, and what does the data say?",
        keyTakeaway: "DCA removes emotional timing errors from investing. Automatic, regular investments capture market downturns as buying opportunities. For regular income, DCA is default-optimal. For lump sums, lump sum wins on average — but only if you execute it.",
        estimatedMinutes: 5,
    },
    {
        id: "fl-006",
        ...
        appliedExample: "Vanguard's Target Retirement 2055 fund automatically holds ~90% stocks, 10% bonds for a 30-year-old, gradually shifting to 50/50 at retirement. During the 2008 financial crisis, a 60/40 portfolio (stocks/bonds) fell ~26% vs the S&P 500 at -55% — bonds acted as a shock absorber. The 60/40 portfolio is still considered the benchmark for balanced investing, though rising correlation between stocks and bonds during 2022 inflation challenged its assumptions.",
        questionRecall: "What is rebalancing, and why does it mechanically force a 'buy low, sell high' discipline?",
        questionApplication: "You're 35 years old with $200,000 in a 401(k), all in stocks. Markets have risen 30% in 2 years and you're nervous. How do you think about rebalancing, and what allocation might be appropriate?",
        keyTakeaway: "Asset allocation — not individual stock picks — drives most investment outcomes. Decide your stock/bond split based on time horizon and risk tolerance, then rebalance annually to maintain it.",
        estimatedMinutes: 5,
    },
    {
        id: "fl-007",
        ...
        appliedExample: "Three debts: Credit Card $3,000 at 22%, Car $12,000 at 6%, Student Loan $25,000 at 4.5%. Avalanche: attack credit card first (saves most interest). Snowball: also attacks credit card first (it's the smallest high-rate debt anyway). If instead the car was $2,000 at 6%: Snowball would attack car, Avalanche would attack credit card — this is where they diverge. Savings: Avalanche saves on average $1,000–$3,000 in interest on typical debt loads vs snowball.",
        questionRecall: "What is the core difference between the Debt Avalanche and Debt Snowball methods? Which is mathematically optimal?",
        questionApplication: "You have: $800 credit card at 28%, $5,000 medical debt at 0%, $15,000 car loan at 7%, $40,000 student loans at 5%. You have $400/month extra. Design your paydown sequence using the avalanche method.",
        keyTakeaway: "Avalanche saves the most money; Snowball saves the most motivation. Choose based on your psychology. For most people, the best strategy is the one they'll actually stick to.",
        estimatedMinutes: 5,
    },
    {
        id: "fl-008",
        ...
        appliedExample: "AAPL is at $150. You buy a call with $155 strike expiring in 30 days for $3 premium (=$300 for 100 shares). If AAPL hits $165, your option is worth $10 intrinsically — you made $700 on a $300 investment (133% gain). If AAPL stays at $150, your option expires worthless — you lose $300. Selling covered calls on stocks you own generates income: own 100 shares of AAPL at $150, sell a $160 call for $2 ($200). If stock stays below $160, you keep $200. If it rises above $160, you sell at $160 (capped upside but still profitable).",
        questionRecall: "What is the difference between a call option and a put option? What does it mean for an option to be 'in the money'?",
        questionApplication: "You own 500 shares of a tech stock at $80. You're worried about a market downturn in the next 3 months but don't want to sell. How could you use put options to protect yourself, and what would you be giving up?",
        keyTakeaway: "Options are leverage instruments: large gains or total loss of premium. Call = right to buy, Put = right to sell. Theta decay works against buyers. Learn covered calls and protective puts before speculative options strategies.",
        estimatedMinutes: 5,
    },
    {
        id: "fl-009",
        ...
        coreConcept: "Real Estate advantages: Leverage (you control $400,000 of property with $80,000 down — 5x leverage), Cash flow (rental income), Tax benefits (depreciation deduction, 1031 exchanges), Inflation hedge (rents and values rise with inflation). Stock advantages: Liquidity (sell in seconds vs months), No management (passive), Lower transaction costs (stocks: 0%, real estate: 6–10%), True diversification (own 3,000 companies for $10). Historical returns: US stocks ≈ 10% annually, real estate price appreciation ≈ 4% (Case-Shiller national index), but total return with rent ≈ 8–10%. The Illiquidity Premium: real estate investors earn extra return partly because the asset is hard to sell — you're compensated for locking up capital.",
        appliedExample: "A $400,000 rental property with $80,000 down, generating $2,000/month rent, $1,600/month costs (mortgage, taxes, insurance, maintenance): $400/month cash flow, or $4,800/year on $80,000 invested = 6% cash-on-cash return, before appreciation. If property appreciates 4%/year, that's $16,000/year gain on $80,000 down = 20% return on equity. Leverage amplifies both gains and losses — if the property drops 20%, your entire down payment is wiped out.",
        questionRecall: "What is 'cash-on-cash return' in real estate, and how does leverage affect it compared to buying stocks outright?",
        questionApplication: "You have $100,000 to invest. Option A: put it all in a total market index fund. Option B: use it as a down payment on a $500,000 rental property. What factors would drive your decision beyond just expected return?",
        keyTakeaway: "Real estate offers leverage, cash flow, and tax benefits; stocks offer liquidity and passive simplicity. Both belong in a diversified portfolio. Real estate's true return includes rent — appreciation alone understates it.",
        estimatedMinutes: 5,
    },
    {
        id: "fl-010",
        ...
        appliedExample: "Common mistakes: closing old credit cards (removes available credit, raising utilization ratio and shortening history), applying for multiple cards quickly (multiple hard inquiries), carrying a balance to 'show activity' (myth — utilization is measured at statement date regardless). Quick wins: pay card statement balance before statement closes (reduces reported utilization), ask for credit limit increases without hard inquiry, become an authorized user on an old account with good history.",
        questionRecall: "What are the five components of a FICO score and their approximate weights?",
        questionApplication: "Your credit score is 680. You want to buy a house in 18 months. You have two cards with 60% utilization and one old card you were thinking of closing. What specific actions do you take to maximize your score before the mortgage application?",
        keyTakeaway: "Payment history and utilization drive 65% of your score. Never close old accounts. Pay down utilization below 10% before major credit applications. One year of consistent on-time payments can move a score 50–80 points.",
        estimatedMinutes: 5,
    },
    // ─── MENTAL HEALTH LITERACY (10) ─────────────────────────────────────────
    {
        ...
        appliedExample: "You present in a meeting and one person looks unengaged. All-or-Nothing: 'That was a disaster.' Mind Reading: 'They think I'm incompetent.' The CBT reframe: 'One person seemed distracted. I don't know why. Others seemed engaged. I can ask for feedback if I'm unsure.' Research by Hofmann et al. (2012) meta-analysis: CBT is effective for depression, anxiety, OCD, PTSD, and more across 269 studies.",
        questionRecall: "What is 'emotional reasoning' as a cognitive distortion? Give an example of how it works in practice.",
        questionApplication: "You receive a critical comment in a code review and immediately think 'I'm a terrible engineer, I shouldn't be in this role.' Identify which cognitive distortions are present and write a more balanced counter-thought.",
        keyTakeaway: "Thoughts are not facts. Cognitive distortions are predictable brain bugs. Naming the distortion ('that's catastrophizing') creates distance from it and is the first step to a more accurate, less painful perspective.",
        estimatedMinutes: 5,
    },
    {
        id: "mh-002",
        ...
        appliedExample: "In conflict resolution: if someone is hyperaroused (voice raised, defensive), logical argument will fail — their prefrontal cortex is offline. First, lower arousal through pacing, pausing, acknowledging feelings ('It sounds like you're frustrated'). Only when they're back in the window can you problem-solve. This is why the adage 'never negotiate angry' has neuroscience behind it. In parenting, a child mid-tantrum cannot access reasoning — co-regulation (calm presence, not reasoning) returns them to the window.",
        questionRecall: "What are the two states outside the Window of Tolerance and what does each feel like?",
        questionApplication: "You're in a heated performance review. You notice your heart racing, your thoughts scattering, and you're about to say something reactive. Using the Window of Tolerance model, what do you understand is happening and what do you do?",
        keyTakeaway: "Stress, sleep deprivation, and trauma narrow your window of tolerance. Outside the window, rational function is impaired. Recognizing when you or others are outside the window — and regulating first, reasoning second — is emotional intelligence in practice.",
        estimatedMinutes: 5,
    },
    {
        id: "mh-003",
        ...
        appliedExample: "You're rejected from a job you really wanted. Without RAIN: either hours of self-criticism (rumination) or numbing with TV/alcohol. With RAIN: Recognize ('I'm feeling shame and disappointment'), Allow (sit with it briefly, don't rush to 'fix'), Investigate ('I feel it in my chest, tight; it's telling me I'm not good enough'), Nurture ('This is genuinely disappointing. It makes sense that I feel this way. Many capable people get rejected. I can try again.'). Brach's research and clinical work shows RAIN interrupts the shame-rumination cycle within minutes.",
        questionRecall: "What does each letter in RAIN stand for, and what does the 'Recognize' step do neurologically?",
        questionApplication: "You make a significant mistake at work and feel a wave of shame. Walk through all four steps of RAIN applied to this specific situation.",
        keyTakeaway: "RAIN processes difficult emotions instead of suppressing or amplifying them. Naming emotion reduces its intensity. Allowing — not resisting — is counter-intuitive but essential. Self-compassion is not weakness; it's what resilient people actually practice.",
        estimatedMinutes: 5,
    },
    {
        id: "mh-004",
        ...
        appliedExample: "John Gottman's 40 years of marriage research: stable couples have approximately a 5:1 positive-to-negative interaction ratio. Not because they avoid conflict, but because they maintain enough positive interactions to buffer the negativity bias. In investing: loss aversion causes investors to sell winners too early (capture gains) and hold losers too long (avoid realizing losses), systematically reducing returns. To counteract negativity bias: deliberately 'install' positive experiences by savoring them for 15–20 seconds (Hanson's 'taking in the good').",
        questionRecall: "What is negativity bias and what evolutionary purpose did it serve? Why is it maladaptive in many modern contexts?",
        questionApplication: "You lead a team. You gave five pieces of positive feedback and one critical correction in a team meeting. A team member leaves feeling demoralized. Using negativity bias, explain what happened and how you'd adjust your communication approach.",
        keyTakeaway: "Negative experiences stick harder than positive ones — by evolutionary design. Counter this by actively savoring positives, understanding that criticism lands 5x harder than praise, and in relationships, maintaining a 5:1 positive ratio.",
        estimatedMinutes: 5,
    },
    {
        id: "mh-005",
        ...
        appliedExample: "In conflict: an anxious person escalates ('Why aren't you responding to me?'), an avoidant person withdraws ('I need space'). This anxious-avoidant dance is the most common conflict pattern in relationships and creates a painful loop: anxious escalation triggers avoidant withdrawal which triggers more anxious escalation. Understanding the dynamic allows both people to name the pattern rather than getting absorbed in content: 'I notice we're in our pattern. I feel anxious, you're pulling away. Can we try to do this differently?'",
        questionRecall: "What are the four attachment styles? Which is the most common and what parenting pattern tends to produce it?",
        questionApplication: "Your partner tends to withdraw and go quiet when you're upset, which makes you escalate and pursue them more intensely. Using attachment theory, analyze what might be happening for each of you and suggest one change each person could make.",
        keyTakeaway: "Attachment styles are learned patterns, not fixed traits. Recognizing your style and your partner's creates compassion for the dynamic rather than blame. Earned secure attachment is possible at any age through awareness and relationships.",
        estimatedMinutes: 5,
    },
    {
        id: "mh-006",
        ...
        appliedExample: "The 2-Minute Rule (James Clear/David Allen): if it takes less than 2 minutes, do it now. More relevantly: commit only to the first 2 minutes of an action. 'I'll just put on my running shoes.' Often the activation energy barrier is what breaks — once started, inertia carries you forward. For severe depression: schedule every hour of the day with low-demand but engaging activities. Monitor mood ratings before/after to build evidence against the depression's 'nothing helps' narrative.",
        questionRecall: "Why does Behavioral Activation tell you to act before you feel like acting? What is the theoretical model behind it?",
        questionApplication: "You've been feeling low and isolated for two weeks. You've stopped exercising, stopped seeing friends, and spend evenings watching TV. Using Behavioral Activation principles, design a realistic plan for the next week.",
        keyTakeaway: "Motivation follows action in depression — not the other way around. Schedule small, specific activities. Don't wait to feel like it. The downward spiral of withdrawal can be reversed by behavioral engagement, starting with the smallest possible step.",
        estimatedMinutes: 5,
    },
    {
        id: "mh-007",
        ...
        coreConcept: "Mindfulness is present-moment, non-judgmental awareness of experience. Rumination is repetitive, passive focus on distress — mental time travel to the past (guilt, regret) or future (worry, anxiety). Killingsworth & Gilbert (2010) tracked 2,250 people via smartphone: 'A wandering mind is an unhappy mind' — mind-wandering predicted worse mood regardless of the activity. Mindfulness research (Jon Kabat-Zinn's MBSR program): 8 weeks reduces anxiety, depression, and pain, increases cortical thickness in prefrontal regions, reduces amygdala reactivity. The key distinction: mindfulness includes thoughts ('I notice I'm thinking about tomorrow's meeting') — it's meta-awareness, not thought suppression. Rumination circles the same content without resolution; mindfulness observes without being captured by content.",
        appliedExample: "The standard MBSR breath exercise: attend to breath sensation. When mind wanders (it will), gently return. Each return is a 'rep' — you're training the prefrontal cortex's ability to override the default mode network (the brain's rumination circuit). 10 minutes daily for 8 weeks shows measurable brain changes. Rumination maintenance: 'Why did I say that?' loops don't produce insight — they produce more rumination. The antidote is either problem-solving (if actionable) or radical acceptance (if not).",
        questionRecall: "What is the difference between mindfulness and rumination? What did the Killingsworth & Gilbert study find about mind-wandering and happiness?",
        questionApplication: "You have an important presentation tomorrow and find yourself repeatedly cycling through worst-case scenarios tonight. Distinguish between productive preparation and rumination, and describe a mindfulness technique to interrupt the cycle.",
        keyTakeaway: "Rumination is passive, repetitive, and increases distress. Mindfulness is active awareness that interrupts rumination without suppressing thoughts. Even 10 minutes daily of breath-focused practice produces measurable reductions in anxiety and depression within weeks.",
        estimatedMinutes: 5,
    },
    {
        id: "mh-008",
        ...
        appliedExample: "Cold showers are hormetic stress: brief cold exposure triggers norepinephrine release (100–300% increase per Huberman Lab review), improves mood, and builds tolerance for discomfort. Progressive overload in exercise is hormesis: muscles micro-tear and rebuild stronger. For anxiety: graduated exposure (starting with less feared situations and building up) is the gold standard — avoiding feared situations strengthens anxiety, facing them (gradually) extinguishes it.",
        questionRecall: "What is the Yerkes-Dodson Law and what does it suggest about the relationship between stress and performance?",
        questionApplication: "You have severe public speaking anxiety. Your instinct is to avoid presenting until you feel ready. Using stress inoculation principles, design a graduated exposure plan over 3 months.",
        keyTakeaway: "Moderate, manageable stress builds resilience; chronic overwhelming stress destroys it. Deliberately seek out small challenges to build capacity. Avoiding stress consistently weakens your response to it.",
        estimatedMinutes: 5,
    },
    {
        id: "mh-009",
        ...
        appliedExample: "In high-stress work environments: teams that celebrate together, share meals, and build genuine relationships are not 'wasting time' — they're building a physiological buffer against stress-related performance degradation. Military units bond intensely not just for morale but because genuine social connection measurably improves resilience under fire. Remote work research consistently finds higher loneliness and burnout — not because remote workers are weaker, but because the social baseline assumption is unmet.",
        questionRecall: "What does Social Baseline Theory mean by the brain 'outsourcing' regulatory functions to others?",
        questionApplication: "You've been working remotely for a year and notice higher stress and worse sleep than when you worked in an office. Using Social Baseline Theory, explain what might be happening physiologically and suggest three concrete interventions.",
        keyTakeaway: "Social connection is a physiological resource, not a luxury. Your nervous system is literally more efficient when in trusted company. Loneliness has health impacts equivalent to smoking. Invest in social connection as seriously as diet and exercise.",
        estimatedMinutes: 5,
    },
    {
        id: "mh-010",
        ...
        appliedExample: "Physiological sigh (double inhale through nose, long exhale through mouth): the extended exhale activates the ventral vagal brake on the heart (through the sinoatrial node), rapidly shifting from sympathetic to ventral vagal. This is the fastest known method for real-time nervous system downregulation. Cold water on face activates the dive reflex — slows heart rate via vagus nerve. Eye contact, prosodic voice (melodic, varied), and safe touch all activate the ventral vagal social engagement system.",
        questionRecall: "What are the three states described by Polyvagal Theory and what does each state feel like?",
        questionApplication: "You're about to walk into a high-stakes negotiation and feel frozen and slightly dissociated (dorsal vagal shutdown). Using Polyvagal Theory, what would you do in the 5 minutes before you walk in to shift your state?",
        keyTakeaway: "The nervous system prioritizes safety signals over reasoning. The vagus nerve is the physiological pathway to calm and connection. Slow exhales, prosodic voice, and safe social contact are tools — not metaphors — for nervous system regulation.",
        estimatedMinutes: 5,
    },
    // ─── PUBLIC SPEAKING (10) ─────────────────────────────────────────────────
    {
        ...
        appliedExample: "Steve Jobs' iPhone introduction (2007) is a masterclass in the rule of three. First, he announced 'three revolutionary products' — an iPod, a phone, an internet communicator. He repeated this three times, building tension. Then: 'These are not three separate devices — this is one device!' The structure created anticipation and revelation. Martin Luther King's 'I Have a Dream' contains dozens of triads. In practical presentations: structure your talk as Problem / Solution / Call to Action — three parts, every time.",
        questionRecall: "Why does the Rule of Three work cognitively? What pattern does the brain expect after two items?",
        questionApplication: "You're pitching a new product to investors. Rewrite this sentence using the Rule of Three: 'Our product is good because it saves time and it's easy to use and it's affordable and it integrates with existing tools.'",
        keyTakeaway: "Three is the most persuasive number in communication. Structure key messages in threes, use tricolons, break complex ideas into three parts. When in doubt, cut to three.",
        estimatedMinutes: 5,
    },
    {
        id: "ps-002",
        ...
        appliedExample: "Martin Luther King Jr's 'I Have a Dream' is the canonical example: he slows dramatically for 'I have a dream,' drops to near-whisper at the emotional peaks, then builds to full volume. The contrast creates the emotional architecture of the speech. Record yourself for 3 minutes and play it back — most people are shocked by how flat they sound. Mark up your script: underline words to stress, put '/' for short pause, '//' for long pause, 'slow' and 'fast' in margins.",
        questionRecall: "What are the four dimensions of vocal variety? Which do most nervous speakers neglect first?",
        questionApplication: "You're delivering a presentation about layoffs to your team. You want to convey both gravity/empathy and confidence/clarity. How would you adjust your pitch, pace, and volume for this specific context?",
        keyTakeaway: "Vocal variety is the delivery equivalent of punctuation — it tells audiences which ideas matter. Practice recording yourself; most people sound more monotone than they feel. Slow down, lower your pitch, and use silence as emphasis.",
        estimatedMinutes: 5,
    },
    {
        id: "ps-003",
        ...
        appliedExample: "Nancy Duarte analyzed hundreds of great speeches and found that the opener establishes the gap between 'what is' and 'what could be' — the tension that drives the entire talk. Simon Sinek's TED Talk: 'How do you explain when things don't go as we assume? Or better, how do you explain why some people and organizations are able to inspire where others aren't?' — opens with a question that promises to resolve a tension everyone has felt. TED's own speaker coaching: first 30 words of your talk should be memorized so thoroughly that extreme nerves cannot disrupt them.",
        questionRecall: "Name three types of effective opening hooks and two common openings you should always avoid.",
        questionApplication: "You're presenting a proposal to adopt a new software tool that will disrupt the team's current workflow. Write an opening hook (3–4 sentences) that would make the audience lean forward rather than cross their arms.",
        keyTakeaway: "The first 30 seconds are the most consequential of any talk. Never waste them on logistics. Memorize your opener. Create a gap — a tension between current reality and possibility — that your talk then resolves.",
        estimatedMinutes: 5,
    },
    {
        id: "ps-004",
        ...
        appliedExample: "Barack Obama is celebrated for his pauses. In his 2004 DNC keynote: he regularly paused 3–5 seconds mid-sentence, particularly before critical words. This created an almost rhythmic quality that kept audiences riveted. Toastmasters' main exercise for nervous speakers: stand at the podium, say nothing, make eye contact, breathe, and count silently to five before beginning. Most speakers report this as the most challenging exercise they've done — and the most transformative. The pause is counterintuitively the cure for the filler word habit.",
        questionRecall: "What are three different types of pauses in public speaking and when would you use each?",
        questionApplication: "You're presenting data that shows your project is 3 months behind schedule. Write out (with pause marks) how you would deliver the sentence: 'The project is behind schedule. But here is why this is actually an opportunity.'",
        keyTakeaway: "Silence is emphasis. A 3-second pause feels eternal to the speaker and completely natural to the audience. Eliminate filler words by replacing them with deliberate pauses. Pause before and after every key idea.",
        estimatedMinutes: 5,
    },
    {
        id: "ps-005",
        ...
        appliedExample: "Cuddy's original finding (power poses affect testosterone/cortisol) did not replicate, but the self-perception effect did: adopting open, expansive posture before a high-stakes situation increased feelings of confidence and reduced anxiety. This is consistent with embodied cognition research. Practical: 2 minutes of expansive posture backstage before speaking shifts your psychological state — even if the hormones don't change as dramatically as originally claimed. Video yourself from the waist up and count gesture frequency: most people gesture too rarely, making them appear stiff.",
        questionRecall: "What are three body language elements that signal confidence to an audience? What signals nervousness?",
        questionApplication: "You're presenting standing at a podium with a fixed microphone. You can't move freely. How do you use posture, gesture, and facial expression to maintain audience engagement?",
        keyTakeaway: "Open posture, purposeful gestures, and eye contact are the three pillars of credible physical presence. Move intentionally or don't move. Adopt an expansive posture backstage before speaking — it shifts your internal state regardless of what it does to your hormones.",
        estimatedMinutes: 5,
    },
    {
        id: "ps-006",
        ...
        appliedExample: "Brené Brown's TED Talk on vulnerability opens with: 'A year after I started my research, I had what I call a breakdown. My therapist called it a spiritual awakening.' This is specific, personal, and immediately vulnerable — creating instant emotional identification. Google's 'Year in Search' ads follow pure hero's journey — person faces devastating challenge, seeks help, finds connection, transforms. Princeton neuroscientist Uri Hasson demonstrated via fMRI that when a speaker tells a personal story, listeners' brain activity (especially in prefrontal and parietal areas) synchronizes with the speaker's — a phenomenon called neural coupling.",
        questionRecall: "What is the minimum structure of a good story? Why is conflict essential — what happens to a story without it?",
        questionApplication: "You need to convince your team to adopt a new process. Instead of presenting bullet points, structure this as a story: the team was struggling, you tried the new process, here's what happened. Write a 3-sentence version using the context/conflict/resolution structure.",
        keyTakeaway: "Stories require conflict — without tension, there's no narrative. Be specific with details (names, places, numbers). Alternate between 'what is' and 'what could be' to move audiences emotionally. The brain processes stories as experience, not information.",
        estimatedMinutes: 5,
    },
    {
        id: "ps-007",
        ...
        appliedExample: "Richard Feynman's Caltech lectures demonstrate masterful Q&A: he almost always restated questions in his own words ('What you're really asking is...'), often simplifying them — this reframe allowed him to answer the deeper question beneath the surface question. Politicians use bridging so consistently you can train yourself to spot it: 'I appreciate that question, but what's really important is...' is a textbook bridge. Preparing Q&A: Jeff Weiner (LinkedIn CEO) reportedly prepared for board meetings by writing out every question he expected and his ideal answer, then having his team stress-test with unexpected ones.",
        questionRecall: "What is the 'bridge technique' in Q&A? When would you use it and what does it sound like?",
        questionApplication: "You've just presented a proposal and someone asks: 'This sounds expensive. Are you sure we can afford this?' You don't have budget numbers memorized. Walk through exactly how you would respond using Q&A best practices.",
        keyTakeaway: "Prepare 20 questions before every important talk. In Q&A: pause, rephrase, answer directly. Use the bridge to redirect hostile questions. 'I don't know, I'll find out' is always better than guessing. Q&A is where trust is often won or lost.",
        estimatedMinutes: 5,
    },
    {
        id: "ps-008",
        ...
        appliedExample: "Barbra Streisand famously avoided live performance for 27 years due to stage fright after forgetting lyrics. Her therapist helped her reframe: the audience wants you to succeed. Survey data: 85% of audience members report rooting for the speaker to do well. They feel empathy when speakers struggle — not schadenfreude. The practical implication: your audience is your ally, not your judge. Before your next talk, say 'I'm excited' aloud three times. It sounds silly; the research shows it works.",
        questionRecall: "What is 'arousal reappraisal' and how is it different from trying to 'calm down' before a presentation?",
        questionApplication: "You have a major presentation in 10 minutes. Your hands are shaking, heart is racing, and you feel like fleeing. Using arousal reappraisal and what you know about performance physiology, walk through exactly how you'd talk to yourself right now.",
        keyTakeaway: "Nervousness and excitement are physiologically identical. Stop fighting your arousal — reframe it as excitement instead. Your body is preparing you for peak performance, not punishment. The audience wants you to succeed.",
        estimatedMinutes: 5,
    },
    {
        id: "ps-009",
        ...
        appliedExample: "Ronald Reagan was nicknamed 'The Great Communicator' — observers noted he made each person in a large room feel personally addressed. Study of his debate tapes shows regular 3–5 second sustained contact with specific audience members across all zones. For virtual calls: put a sticky note with a smiley face next to your webcam. This reminds you to look at the camera (where your virtual eye contact lives) rather than at the faces on screen (which produces the appearance of looking down).",
        questionRecall: "Describe the Lighthouse Method for eye contact. How long should you hold eye contact with one person before moving?",
        questionApplication: "You're presenting to 8 people around a conference table. You tend to look at your most senior stakeholder most of the time. Using the lighthouse method, how would you distribute your eye contact differently?",
        keyTakeaway: "Complete a thought while holding one person's gaze; move to a new person for the next thought. 3–5 seconds per person feels warm; less feels avoidant. For virtual presentations, the camera is your eye contact — not the faces on screen.",
        estimatedMinutes: 5,
    },
    {
        id: "ps-010",
        ...
        appliedExample: "Question: 'Should we expand into the European market?' Without PREP: 'Well, there are a lot of factors to consider. The European market has some regulatory challenges like GDPR, but also there's potential revenue, though our team is stretched... I think maybe yes?' With PREP: 'Yes, we should expand into Europe. [Point] The market opportunity is three times our current TAM, and two competitors are already moving there. [Reason] Our product has already been purchased by 12 European companies without marketing effort. [Example] So yes — I recommend we begin a scoped pilot in Germany and Netherlands this year. [Point]' Same content; completely different perceived competence.",
        questionRecall: "What does PREP stand for and why does it start with the Point rather than building up to it?",
        questionApplication: "Someone asks you in a meeting: 'What do you think about switching our entire product from subscription to usage-based pricing?' You have 30 seconds. Use PREP to structure a response.",
        keyTakeaway: "State your conclusion first, then support it. PREP (Point, Reason, Example, Point) transforms improvised answers into organized responses. Practice it in low-stakes conversations — answer emails and Slack messages in PREP format to build the habit.",
        estimatedMinutes: 5,
    },
    // ─── CLIMATE LITERACY (10) ────────────────────────────────────────────────
    {
        ...
        appliedExample: "The Paris Agreement (2015): 195 countries agreed to limit warming to 'well below 2°C' with efforts toward 1.5°C. Current Nationally Determined Contributions (NDCs) put us on track for approximately 2.5–2.7°C by 2100. The gap between pledges and required action is called the 'emissions gap' — UNEP calculates countries need to roughly triple their ambition. Carbon budgets help translate abstract temperatures into concrete tonnage limits that can be tracked, allocated, and enforced.",
        questionRecall: "What is a carbon budget and how does the 1.5°C carbon budget compare to current annual global emissions?",
        questionApplication: "A country emits 500 million tonnes of CO2 per year and has pledged to reach net zero by 2050. Its 'fair share' of the 1.5°C budget is 8 billion tonnes total. Is its 2050 net-zero pledge consistent with the 1.5°C budget? Show your reasoning.",
        keyTakeaway: "The carbon budget is finite and nearly exhausted. Cumulative emissions determine warming — not the rate in any given year. Every year of delay shrinks the remaining budget and makes the required annual reductions steeper.",
        estimatedMinutes: 5,
    },
    {
        id: "cl-002",
        ...
        appliedExample: "The Siberian permafrost contains more stored carbon than all the coal reserves on Earth. As permafrost thaws, ancient organic matter decomposes, releasing CO2 and methane (80x more potent than CO2 over 20 years). This creates a self-amplifying feedback: warming thaws permafrost, which releases GHGs, which causes more warming, which thaws more permafrost. Once triggered past a threshold, this process cannot be stopped by human emission reductions — a true planetary runaway. This is why climate scientists are more alarmed than the 'average 2°C is manageable' framing suggests.",
        questionRecall: "What makes a climate tipping point different from gradual, reversible climate change? Give two examples of tipping points.",
        questionApplication: "A policymaker argues: 'Even if we overshoot 1.5°C, we can use carbon capture to bring CO2 back down later.' Using tipping point science, what is the strongest counterargument to this?",
        keyTakeaway: "Tipping points are irreversible thresholds, not gradual changes. Several may trigger below 2°C of warming, and they can cascade. The risk of triggering tipping points is a key reason why preventing overshoot is fundamentally different from temporarily overshooting and then reducing emissions.",
        estimatedMinutes: 5,
    },
    {
        id: "cl-003",
        ...
        appliedExample: "The cost problem: current DAC costs ~$400–1,000/tonne. Oil companies profit from selling a tonne of CO2-equivalent at ~$20–50 (as fossil fuel revenue). Economic viability requires massive cost reduction (target: $100/tonne) or carbon pricing that makes emitting expensive. Stripe, Shopify, and Alphabet are early buyers through advance purchase agreements, funding the learning curve. Enhanced weathering (spreading silicate rock dust on farmland) shows promise at potentially $50–200/tonne and co-benefits for agriculture.",
        questionRecall: "What is Direct Air Capture (DAC) and what is the current scale gap between DAC capacity and global emissions?",
        questionApplication: "A company claims to be 'carbon neutral' by purchasing DAC credits to offset all its emissions. What questions would you ask to evaluate whether this claim represents genuine climate action?",
        keyTakeaway: "Carbon removal is necessary but not sufficient — it complements steep emission cuts, it doesn't replace them. Current DAC cost and scale make it a minor contribution for now. Natural sinks are being degraded at the same time they're most needed.",
        estimatedMinutes: 5,
    },
    {
        id: "cl-004",
        ...
        appliedExample: "The renewable energy cost collapse has been one of the most wrong-predicted trends in history: the IEA's 2010 World Energy Outlook projected solar would cost $0.21/kWh by 2050; it reached $0.044 by 2023. This systematic underestimation happened because modelers didn't account for learning curve dynamics. The transition economics are now reversed: in most markets, it's cheaper to build new wind/solar than to operate existing coal plants. Texas added more wind capacity than any state between 2000–2022 purely on economics, not environmental policy.",
        questionRecall: "What is LCOE and how does solar compare to coal on this metric in 2023?",
        questionApplication: "A utility company argues it should keep its coal plant running because it's 'already paid for.' Using LCOE analysis, what economic argument could you make against this position?",
        keyTakeaway: "Renewables are now the cheapest source of new electricity generation globally. This is economics, not idealism. Wright's Law suggests costs will continue falling as deployment scales. The remaining challenge is grid integration — storage, transmission, and backup capacity.",
        estimatedMinutes: 5,
    },
    {
        id: "cl-005",
        ...
        appliedExample: "ICCT (2021) study across Europe, US, China, and India: battery EVs emit 66–69% lower lifecycle GHGs in Europe, 60–68% in the US, 37–45% in China, and 19–34% in India over the vehicle's lifetime. The key variable is grid carbon intensity. Tesla's Model 3 manufactured in Shanghai with Chinese grid electricity has higher lifecycle emissions than one manufactured in Nevada — same car, different supply chain carbon footprint. This underscores why grid decarbonization and EV adoption are complementary policies.",
        questionRecall: "What is the 'carbon debt' of manufacturing an EV, and what determines how quickly it is repaid?",
        questionApplication: "You live in a state where 70% of electricity comes from natural gas. Your neighbor says 'EVs are just coal cars — they just move the emissions to the power plant.' How do you respond with the lifecycle analysis data?",
        keyTakeaway: "EVs are lower-lifecycle-emission vehicles in almost every grid context, and they get cleaner as grids decarbonize. The manufacturing carbon debt is real but is repaid within 1–3 years of driving. Grid decarbonization multiplies the climate benefit of every EV on the road.",
        estimatedMinutes: 5,
    },
    {
        id: "cl-006",
        ...
        appliedExample: "Natural gas is marketed as a 'clean' bridge fuel because it produces ~50% less CO2 than coal when burned. But methane leakage from extraction, pipelines, and distribution undermines this advantage: a leakage rate above ~3.2% makes natural gas worse than coal on a 20-year GWP basis. Studies of US natural gas infrastructure find leakage rates of 2.3–3.7% (Alvarez et al., Science, 2018) — right at or above the break-even point. This is the 'methane problem' that makes natural gas expansion environmentally questionable even as a transition fuel.",
        questionRecall: "Why is methane's 20-year GWP more relevant than its 100-year GWP for near-term climate policy? What are the three largest sources of methane emissions?",
        questionApplication: "An energy company argues that switching from coal to natural gas power plants reduces climate impact by 50% because gas burns cleaner. What is the key piece of information missing from this argument, and how might it change the conclusion?",
        keyTakeaway: "Methane is 80x more potent than CO2 over 20 years but is short-lived — cutting it produces fast climate benefits. Fossil fuel methane leaks may undermine natural gas's climate advantage over coal. Reducing methane is one of the fastest available levers for slowing near-term warming.",
        estimatedMinutes: 5,
    },
    {
        id: "cl-007",
        ...
        appliedExample: "The Pacific Northwest oyster industry lost 80% of its hatchery production in 2008 when abnormally acidic water upwelled along the coast — one of the first economic casualties of ocean acidification. NOAA research in the Southern Ocean found pteropod shells dissolving in real-time in water that is already below the aragonite saturation threshold. The Great Barrier Reef has bleached four times since 2016 (1998, 2002, 2016, 2017, 2020, 2022) — bleaching used to occur every 25–30 years. At 2°C warming, 99% of coral reefs are projected to experience annual bleaching.",
        questionRecall: "What chemical process causes ocean acidification, and why is the 0.1-unit pH drop more significant than it sounds?",
        questionApplication: "A friend says 'Marine life has survived pH changes before over millions of years, so this is nothing new.' How do you respond using what you know about the rate of change and its biological significance?",
    ...
    },
    {
        ...
        appliedExample: "Kiribati is a Pacific island nation of 119,000 people contributing negligible GHG emissions. Its highest point is 2 meters above sea level. At current warming trajectories, it will be uninhabitable by 2050–2100. The government is pre-emptively purchasing land in Fiji and training citizens for migration. The Marshall Islands is negotiating to maintain statehood even as its physical territory disappears. The Inuit Circumpolar Council submitted a petition to the Inter-American Commission on Human Rights in 2005, arguing US emissions violated Inuit human rights — a landmark legal framing of climate as a human rights issue.",
        questionRecall: "What is 'Loss and Damage' in climate negotiations, and why is it distinct from adaptation finance?",
        questionApplication: "A wealthy country argues it shouldn't pay into the Loss and Damage Fund because it has already reduced its own emissions significantly. Using the principle of historical responsibility, construct the counterargument.",
        keyTakeaway: "Climate impact and climate responsibility are inversely distributed globally. Those least responsible for emissions face the worst consequences. Climate justice — centering frontline communities and historical accountability — is inseparable from effective climate policy.",
        estimatedMinutes: 5,
    },
    {
        id: "cl-009",
        ...
        appliedExample: "The psychological danger of 'personal responsibility' framing: it creates moral licensing (I recycled, so I can fly) and despair (my actions are too small to matter). The more accurate framing: individual high-impact choices matter AND systemic change is where the multiplier is. Flying less matters (it's one of the highest per-person impacts). Voting for carbon pricing matters more (it changes the economics for millions). Shareholder activism matters (divesting public universities from fossil fuels shifts capital markets). Both/and, not either/or.",
        questionRecall: "Which three individual actions have the highest actual CO2 impact according to Wynes & Nicholas (2017)? Which common actions are surprisingly low-impact?",
        questionApplication: "A colleague says 'I use a metal straw and compost, so I'm doing my part.' Using what you know about carbon footprint magnitude and system change, how do you respond constructively — neither dismissing their efforts nor leaving them satisfied with low-impact actions?",
...
export function searchLessons(query, domain) {
    const q = query.toLowerCase();
    let pool = domain ? getLessonsByDomain(domain) : LESSONS;
    return pool.filter((l) => l.conceptName.toLowerCase().includes(q) ||
